...
import { User, Settings, LogOut, ChevronLeft, CalendarCheck, Phone, LayoutDashboard, Component, Briefcase, IdCard } from 'lucide-react';
...
export default function ManagerProfilePage() {
    ...
    const fetchDonoProfile = useCallback(async () => {
        ...
        try {
            ...
            console.log('Frontend: Dados recebidos do backend para o perfil:', backendData);
            ...
            setProfileData({
                ...
                razaoSocial: backendData.razaoSocial || 'Não informado',
                cpf: backendData.cpf || 'Não informado',
            });
            ...
            if (err.response) {
                ...
                if (err.response.data && typeof err.response.data === 'string' && err.response.data.includes("No static resource")) {
                    console.error("POSSÍVEL CAUSA: O backend não tem um endpoint GET mapeado para '/api/dono/by-email/{email}'. Verifique o @RequestMapping e @GetMapping no DonoController.java.");
                } else if (err.response.data && typeof err.response.data === 'string' && err.response.data.includes("rawPassword cannot be null")) {
                    console.error("ERRO: Senha nula no backend. Verifique se o campo 'password' no seu DonoRequest.java e UsuarioRequest.java (se existir) está correto, e se o método build() está a passar a senha para o Usuario.");
    ...
    }, [isAuthenticated, navigate, userEmail, authToken, isAuthReady, logout, fetchDonoProfile, profileData]);
    ...
    if (error) {
        return (
            <div style={{ backgroundColor: '#0f172a', color: '#ffffff', minHeight: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', fontSize: '1.2em', padding: '20px', textAlign: 'center' }}>
                ...
                <button onClick={fetchDonoProfile} style={{
